			...
			uint32_t *otp_len)
{
	...
	ret = bsec_find_otp_name_in_dt(otp_name, otp_idx, otp_len);
	if (ret != BSEC_OK) {
		ERROR("Failed to find '%s' in DTB: %d\n", otp_name, ret);
	}
